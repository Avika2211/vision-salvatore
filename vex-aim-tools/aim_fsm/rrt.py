...
from .worldmap import BarrelObj, SportsBallObj, AprilTagObj, ArucoMarkerObj, WallObj, DoorwayObj, RoomObj, DominoObj
...
class RRT():
    ...
    def generate_obstacles(self, goal_object,
        ...
        for obj in self.robot.world_map.objects.values():
            ...
            elif isinstance(obj, DominoObj):
                obst = self.generate_domino_obstacle(obj, obstacle_inflation)
    ...
    @staticmethod
    def generate_domino_obstacle(domino, inflation=0):
        r = Rectangle(center=geometry.point(domino.pose.x, domino.pose.y),
                      dimensions=[domino.length + 2*inflation, domino.width + 2*inflation],
                      orient=domino.pose.theta or 0)
        r.obstacle_id = domino.id
        return r
